...
import { Info, NotebookPen, Plus, Stethoscope } from "lucide-react";
...
import { byFilter, groupByMonth, recordSections, timeline, type RecordFilter } from "./groupRecords";
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="sticky top-14 md:top-16 z-20 -mx-4 px-4 md:mx-0 md:px-0 py-2 bg-surface/95 backdrop-blur flex flex-wrap items-center gap-x-3 gap-y-2">
        ...
        <div className="flex shrink-0 items-center gap-2">{addButton}</div>
      </div>
              ...
              groups.map((g) => (
                <section key={g.key} aria-label={fmtMonthYear(tc, g.key)}>
                  ...
                  <div className="divide-y divide-line">
                    {g.items.map((r) => (
                      <RecordEntry key={r.id} record={r} viewer={role} />
                    ))}
                  </div>
                </section>
              ))
